...
MODULE       : 12_trigram_markov_field.py
FORMULA NAME : Higher-Order Trigram Markov Field
DESCRIPTION  : Model bahasa statistik tertib kedua yang memodelkan kebarangkalian
               bersama Trigram P(D2 | D0, D1) dan P(D3 | D1, D2) bagi mengunci
               struktur 3-digit serentak.
...
TEMP_OUTPUT_FILE = os.path.join(TEMP_DIR, "recommendations_12_trigram_markov_field.json")
...
def generate_trigram_recommendations(history_draws, top_n=10):
    """
    FORMULA 12: Higher-Order Trigram Markov Field
    Menjana 10 nombor dengan skor rantaian trigram tertinggi.
    """
    ...
    total_uni = sum(unigram.values()) or 1.0
    
    candidates = []
    for d0 in range(10):
        p_d0 = (unigram[d0] + 0.1) / (total_uni + 1.0)
        for d1 in range(10):
            # P(D1 | D0)
            denom_bg = (unigram[d0] + 1.0)
            p_d1_given_d0 = (bigram_01.get((d0, d1), 0.0) + 0.05) / denom_bg
            
            for d2 in range(10):
                # P(D2 | D0, D1)
                denom_tri1 = (bigram_01.get((d0, d1), 0.0) + 0.5)
                p_d2_given_d0d1 = (trigram_012.get((d0, d1, d2), 0.0) + 0.02) / denom_tri1
                
                for d3 in range(10):
                    # P(D3 | D1, D2)
                    denom_tri2 = (bigram_01.get((d1, d2), 0.0) + 0.5)
                    p_d3_given_d1d2 = (trigram_123.get((d1, d2, d3), 0.0) + 0.02) / denom_tri2
                    
                    joint_prob = p_d0 * p_d1_given_d0 * p_d2_given_d0d1 * p_d3_given_d1d2
                    candidates.append((joint_prob, f"{d0}{d1}{d2}{d3}"))
...
def main():
    ...
    print("=" * 80)
    print(f" SIMULASI FORMULA 12: Higher-Order Trigram Markov Field")
    ...
    for i, current_draw in enumerate(testing_draws):
        ...
        recs = generate_trigram_recommendations(historical_window, top_n=10)
        
        latest_recs_payload = {
            "formula_id": "12_trigram_markov_field",
            "formula_name": "Higher-Order Trigram Markov Field",
    ...
    print("=" * 80)
    print(" KEPUTUSAN PRESTASI PELABURAN 6 BULAN (FORMULA 12)")
    print("=" * 80)
