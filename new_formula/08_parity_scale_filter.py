...
MODULE       : 08_parity_scale_filter.py
FORMULA NAME : Parity & High-Low Matrix
DESCRIPTION  : Mengunci nisbah kebarangkalian optimum 2 Ganjil / 2 Genap serta
               2 Rendah (0-4) / 2 Tinggi (5-9) dan menggabungkannya dengan
               kekerapan posisi berwajaran.
...
TEMP_OUTPUT_FILE = os.path.join(TEMP_DIR, "recommendations_08_parity_scale_filter.json")
...
def get_parity_scale_profile(digits):
    """
    Mengira profil Pariti (Ganjil/Genap) dan Skala (Rendah/Tinggi):
    ...
    return (odd_count, even_count), (low_count, high_count)

def generate_parity_scale_recommendations(history_draws, top_n=10):
    """
    FORMULA 08: Parity & High-Low Matrix
    Menjana 10 nombor dengan penapis keutamaan nisbah seimbang 2O:2E dan 2L:2H.
    """
    if not history_draws:
        return []
        
    pos_weights = [defaultdict(float) for _ in range(4)]
    parity_pattern_weights = defaultdict(float)
    scale_pattern_weights = defaultdict(float)
    ...
    for draw in history_draws:
        ...
        for num_str, w in items:
            ...
            if len(num_str) == 4 and num_str.isdigit():
                ...
                for pos in range(4):
                    pos_weights[pos][digits[pos]] += w
                    
                parity_prof, scale_prof = get_parity_scale_profile(digits)
                parity_pattern_weights[parity_prof] += w
                scale_pattern_weights[scale_prof] += w
    ...
    total_par = sum(parity_pattern_weights.values()) or 1.0
    total_scl = sum(scale_pattern_weights.values()) or 1.0
    ...
    for d0 in range(10):
        for d1 in range(10):
            for d2 in range(10):
                for d3 in range(10):
                    digits = [d0, d1, d2, d3]
                    par_prof, scl_prof = get_parity_scale_profile(digits)
                    
                    par_factor = (parity_pattern_weights.get(par_prof, 0.0) + 0.1) / total_par
                    scl_factor = (scale_pattern_weights.get(scl_prof, 0.0) + 0.1) / total_scl
                    
                    # Pengganda khas untuk Zon Emas (2O:2E dan 2L:2H)
                    golden_balance_mult = 1.0
                    if par_prof == (2, 2):
                        golden_balance_mult *= 1.4
                    if scl_prof == (2, 2):
                        golden_balance_mult *= 1.4
                    ...
                    score = base_prob * par_factor * scl_factor * golden_balance_mult
...
def main():
    ...
    print("=" * 80)
    print(f" SIMULASI FORMULA 08: Parity & High-Low Matrix")
    ...
    for i, current_draw in enumerate(testing_draws):
        ...
        recs = generate_parity_scale_recommendations(historical_window, top_n=10)
        
        latest_recs_payload = {
            "formula_id": "08_parity_scale_filter",
            "formula_name": "Parity & High-Low Matrix",
    ...
    print("=" * 80)
    print(" KEPUTUSAN PRESTASI PELABURAN 6 BULAN (FORMULA 08)")
    print("=" * 80)
