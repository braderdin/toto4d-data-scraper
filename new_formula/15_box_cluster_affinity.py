...
MODULE       : 15_box_cluster_affinity.py
FORMULA NAME : 4-Digit Jaccard Cluster Affinity
DESCRIPTION  : Menganalisis graf pertalian kelompok digit (Jaccard Index Affinity)
               merentasi 23 set hadiah bagi mengenalpasti famili kombinasi kotak
               (box clusters) yang mempunyai daya penumpuan tertinggi.
...
TEMP_OUTPUT_FILE = os.path.join(TEMP_DIR, "recommendations_15_box_cluster_affinity.json")
...
def generate_cluster_affinity_recommendations(history_draws, top_n=10):
    """
    FORMULA 15: 4-Digit Jaccard Cluster Affinity
    Menjana 10 nombor berasaskan skor afiniti kelompok Jaccard & posisi berwajaran.
    """
    ...
    triplet_affinity = defaultdict(float)
    ...
    for idx, draw in enumerate(history_draws):
        recency_w = 1.0 + (idx / total_draws) * 0.6
        ...
        # Ekstrak graf korelasi dalam cabutan yang sama
        draw_digit_sets = []
        for num_str, tier_w in items:
            ...
            if len(num_str) == 4 and num_str.isdigit():
                ...
                draw_digit_sets.append((set(d), w))
                
                for pos in range(4):
                    pos_weights[pos][d[pos]] += w
                    
                # Pairwise co-occurrence dalam satu nombor
                for p1 in range(4):
                    for p2 in range(p1 + 1, 4):
                        pair = tuple(sorted([d[p1], d[p2]]))
                        pair_affinity[pair] += w
                        
                # Triplet co-occurrence
                for p1 in range(4):
                    for p2 in range(p1 + 1, 4):
                        for p3 in range(p2 + 1, 4):
                            triplet = tuple(sorted([d[p1], d[p2], d[p3]]))
                            triplet_affinity[triplet] += w
    ...
    total_pair_aff = sum(pair_affinity.values()) or 1.0
    total_triplet_aff = sum(triplet_affinity.values()) or 1.0
        ...
        for d1 in range(10):
            for d2 in range(10):
                for d3 in range(10):
                    ...
                    # Skor Afiniti Jaccard Pasangan & Triplet
                    aff_score = 0.0
                    for p1 in range(4):
                        for p2 in range(p1 + 1, 4):
                            pair = tuple(sorted([digits[p1], digits[p2]]))
                            aff_score += (pair_affinity.get(pair, 0.0) / total_pair_aff)
                            
                    trip_score = 0.0
                    for p1 in range(4):
                        for p2 in range(p1 + 1, 4):
                            for p3 in range(p2 + 1, 4):
                                trip = tuple(sorted([digits[p1], digits[p2], digits[p3]]))
                                trip_score += (triplet_affinity.get(trip, 0.0) / total_triplet_aff)
                                
                    combined_score = pos_score * ((1.0 + aff_score * 2.0) ** 0.5) * ((1.0 + trip_score * 3.0) ** 0.5)
                    candidates.append((combined_score, num_str))
...
def main():
    ...
    print("=" * 80)
    print(f" SIMULASI FORMULA 15: 4-Digit Jaccard Cluster Affinity")
    ...
    for i, current_draw in enumerate(testing_draws):
        ...
        recs = generate_cluster_affinity_recommendations(historical_window, top_n=10)
        
        latest_recs_payload = {
            "formula_id": "15_box_cluster_affinity",
            "formula_name": "4-Digit Jaccard Cluster Affinity",
    ...
    print("=" * 80)
    print(" KEPUTUSAN PRESTASI PELABURAN 6 BULAN (FORMULA 15)")
    print("=" * 80)
