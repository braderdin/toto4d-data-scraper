...
DESCRIPTION  : Penalaan lanjutan bagi Formula 05 & 35:
               - Ambang Entropi dilaraskan ke 9.17 untuk sensitiviti optimum.
               - Mod Keyakinan Tinggi : RM 18.00 (12 Nombor)
               - Mod Defensif Seimbang : RM 10.00 (7 Nombor: Top 3 Direct+iBox, Top 4 iBox)
...
TEMP_OUTPUT_FILE = os.path.join(TEMP_DIR, "recommendations_36_tuned_dynamic_ema_gate.json")
...
def generate_tuned_dynamic_ema_recs(history_draws):
    ...
    # 3. Pintu Ambang Entropi Ditala ke 9.17
...
def main():
    ...
    print("=" * 80)
    print(f" SIMULASI FORMULA 36: Tuned Dynamic Exponential Decay Gate")
    print(f" Jumlah Rekod: {total_records} | Latihan: {len(training_draws)} | Ujian: {len(testing_draws)}")
    print(f" Mod Taruhan: Keyakinan Tinggi (RM18.00 / 12 No) vs Defensif (RM10.00 / 7 No)")
    print("=" * 80)
    ...
    for i, current_draw in enumerate(testing_draws):
        ...
        recs, entropy_val, mode_name = generate_tuned_dynamic_ema_recs(historical_window)
    ...
    print("=" * 80)
    print(" KEPUTUSAN PRESTASI PELABURAN 6 BULAN (FORMULA 36)")
    print("=" * 80)
