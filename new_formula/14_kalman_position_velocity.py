...
MODULE       : 14_kalman_position_velocity.py
FORMULA NAME : Kalman Position Velocity Tracking
DESCRIPTION  : Penapis Kalman ruang keadaan (State-Space Filter) untuk menjejak
               posisi dan halaju anjakan (velocity drift) siri digit pada setiap
               kedudukan 4D.
...
TEMP_OUTPUT_FILE = os.path.join(TEMP_DIR, "recommendations_14_kalman_position_velocity.json")
...
class Kalman1DDigitTracker:
    """Penapis Kalman 2-State (Position + Velocity) untuk satu kedudukan digit."""
    def __init__(self, process_var=0.5, measure_var=2.0):
        # State: x = [pos, vel]^T
        self.x = 4.5
        self.v = 0.0
        # Covariance Matrix P
    ...
    def update(self, z):
        ...
        v_pred = self.v
        p00_pred = self.p00 + self.p01 + self.p10 + self.p11 + self.q
        p01_pred = self.p01 + self.p11
        p10_pred = self.p10 + self.p11
        p11_pred = self.p11 + self.q * 0.1
        
        # 2. Measurement Innovation
        # Handle shortest angular distance on circular mod 10
        diff = (z - x_pred)
        # unwrap diff to [-5, 5]
        diff = (diff + 5) % 10 - 5
        ...
        # Update state
        self.x = (x_pred + k0 * diff) % 10.0
        self.v = v_pred + k1 * diff
    ...
    def predict_next(self):
        pred_x = (self.x + self.v) % 10.0
        pred_var = max(self.p00 + self.q, 0.5)
        return pred_x, pred_var

def generate_kalman_recommendations(history_draws, top_n=10):
    """
    FORMULA 14: Kalman Position Velocity Tracking
    Menjana 10 nombor berasaskan ramalan min dan varians penapis Kalman.
    """
    if len(history_draws) < 5:
        return []
        
    trackers = [Kalman1DDigitTracker(process_var=0.6, measure_var=2.5) for _ in range(4)]
    
    # Suapkan data siri masa dari awal ke akhir
    for draw in history_draws:
        p1 = str(draw.get('1st_prize', '')).strip()
        if len(p1) == 4 and p1.isdigit():
            for pos in range(4):
                trackers[pos].update(float(p1[pos]))
                
    # Dapatkan unjuran seterusnya (Pred Mean & Var)
    pos_distributions = [{} for _ in range(4)]
    for pos in range(4):
        mu, var = trackers[pos].predict_next()
        sigma = math.sqrt(var)
        for d in range(10):
            # Modular circular distance
            dist = abs((d - mu + 5) % 10 - 5)
            prob = math.exp(-(dist ** 2) / (2 * (sigma ** 2)))
            pos_distributions[pos][d] = prob
    ...
    for d0 in range(10):
        for d1 in range(10):
            for d2 in range(10):
                for d3 in range(10):
                    score = (
                        pos_distributions[0][d0] * 
                        pos_distributions[1][d1] * 
                        pos_distributions[2][d2] * 
                        pos_distributions[3][d3]
                    )
...
def main():
    ...
    print("=" * 80)
    print(f" SIMULASI FORMULA 14: Kalman Position Velocity Tracking")
    ...
    for i, current_draw in enumerate(testing_draws):
        ...
        recs = generate_kalman_recommendations(historical_window, top_n=10)
        
        latest_recs_payload = {
            "formula_id": "14_kalman_position_velocity",
            "formula_name": "Kalman Position Velocity Tracking",
    ...
    print("=" * 80)
    print(" KEPUTUSAN PRESTASI PELABURAN 6 BULAN (FORMULA 14)")
    print("=" * 80)
