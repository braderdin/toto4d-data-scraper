...
MODULE       : 41_ensemble_f18_tuned_hybrid_direct_ibox.py
FORMULA NAME : Formula 41 - Ensemble Multi-Regime with F18 (1-3M Tuned) Hybrid
DESCRIPTION  : Ensembel 4 formula teras F39 dengan penalaan khas Formula 18:
               1. F18 - Dirichlet Bayesian Momentum ditala tepat ke zon 1-3 Bulan:
                  - Tetingkap Pendek: 13 cabutan (~1 Bulan momentum pantas)
                  - Tetingkap Panjang: 40 cabutan (~3 Bulan sauh penstabilan)
...
STRATEGI PERTARUHAN HIBRID (TEPAT 25 NOMBOR - RM 25.00 / SESI):
  - BIG Direct RM 1.00 (4 Nombor): 1x AAAB, 1x AABB, 1x AABC, 1x ABCD
  - BIG iBox RM 1.00   (21 Nombor): 7x AAAB, 10x AABB, 2x AABC, 2x ABCD

PENILAIAN & OUTPUT:
  - Walk-Forward Backtesting 3 Bulan Terkini (~92 Hari / 42 Sesi).
  - Penjejakan Hari Cabutan (Rabu, Sabtu, Ahad, Selasa).
  - Profiling Penuh 25 Rank Nombor (Kenaan, Modal, Pulangan, PnL, ROI).
...
TEMP_OUTPUT_FILE = os.path.join(TEMP_DIR, "recommendations_41_ensemble_hybrid_direct_ibox.json")
...
# ==============================================================================
# ENJIN MATEMATIK FORMULA 18 (DITALA: 1 BULAN VS 3 BULAN SAHAJA)
# ==============================================================================
def compute_formula_18_engine(history_draws):
    """
    F18 DITALA: Menghadkan horizon kepada tetingkap emas 1 hingga 3 bulan:
    - Long Window  : Maksimum 40 cabutan (~3 bulan) sebagai asas Dirichlet
    - Short Window : Maksimum 13 cabutan (~1 bulan) sebagai momentum pantas
    """
    total_draws = len(history_draws)
    # Hadkan data F18 maksimum kepada 40 cabutan (~3 bulan)
    f18_context = history_draws[-40:] if total_draws >= 40 else history_draws
    ctx_len = len(f18_context)

    short_len = min(13, ctx_len)
...
# ==============================================================================
# ENJIN F20, F22, F36 (KEKAL SEPENUHNYA DARI FORMULA 39)
# ==============================================================================
...
# ==============================================================================
# PENJANAAN 25 NOMBOR HIBRID ENSEMBEL
# ==============================================================================
def generate_formula_41_recommendations(history_draws):
    ...
    # -------------------------------------------------------------
    # A. PILIH 4 NOMBOR DIRECT (TOP 1 DARI SETIAP KATEGORI CORAK)
    # -------------------------------------------------------------
    direct_selected = []
    cat_titles = {
    ...
    # 1. 7x 4-Way Triplet (7 digit berbeza)
    triplet_ibox = []
    ...
    for score, num in candidates_by_perm[4]:
        ...
        if triplet_digit in used_triplet_digits and len(triplet_ibox) < 7:
            continue
...
def main():
    ...
    if HAS_RICH:
        panel_text = Text()
        panel_text.append("🚀 FORMULA 41: ENSEMBLE HYBRID MASTER (F18 1-3 BULAN TUNED)\n", style="bold yellow")
        ...
        panel_text.append("⚡ Sub-Enjin F18 Ditala       : Tetingkap Emas 1-3 Bulan Sahaja (13 Short vs 40 Long)\n", style="bright_cyan")
        ...
        print("=" * 80)
        print(" FORMULA 41: ENSEMBLE HYBRID MASTER (F18 1-3 BULAN TUNED)")
        print(f" Latihan Konteks : {len(initial_training_draws)} sesi ({start_train_str} -> {end_train_str})")
    ...
    for i, current_draw in enumerate(testing_draws):
        ...
        recs, meta = generate_formula_41_recommendations(rolling_context)
        ...
        latest_payload = {
            "formula_id": "41_ensemble_f18_tuned_hybrid_direct_ibox",
            "formula_name": "Formula 41 - Ensemble Multi-Regime with F18 (1-3M Tuned) Hybrid",
    ...
    # ==============================================================================
    # JADUAL RINGKASAN PRESTASI F39 ASAL
    # ==============================================================================
    if HAS_RICH:
        summary_table = Table(title="📊 RINGKASAN PRESTASI 3 BULAN TERKINI (FORMULA 41)", box=box.HEAVY_EDGE, header_style="bold magenta")
        ...
        print("\n" + "=" * 80)
        print(" RINGKASAN PRESTASI 3 BULAN TERKINI (FORMULA 41)")
        print("=" * 80)
