...
MODULE       : 13_permutation_yield_maximizer.py
FORMULA NAME : Permutation Yield Optimizer
DESCRIPTION  : Mengoptimumkan jangkaan hasil pulangan (Expected Yield) dengan
               mengimbangi kebarangkalian kenaan bersama pengganda hasil permutasi
               (12-way dan 6-way double numbers) untuk melipatgandakan pulangan iBox.
...
TEMP_OUTPUT_FILE = os.path.join(TEMP_DIR, "recommendations_13_permutation_yield_maximizer.json")
...
def generate_yield_maximizer_recommendations(history_draws, top_n=10):
    """
    FORMULA 13: Permutation Yield Optimizer
    Menjana 10 kombinasi yang memaksimumkan nisbah kebarangkalian terhadap pembahagi iBox.
    """
    ...
    double_digit_freq = defaultdict(float)
    ...
    for idx, draw in enumerate(history_draws):
        # Pemberat eksponen ringkas untuk tren semasa
        t_weight = 1.0 + (idx / total_draws) * 0.5
        ...
        for num_str, tier_w in items:
            ...
            if len(num_str) == 4 and num_str.isdigit():
                ...
                # Rekod kekerapan digit berulang (kembar)
                for d, cnt in counts.items():
                    if cnt >= 2:
                        double_digit_freq[int(d)] += w * cnt
                        
                for pos in range(4):
                    pos_weights[pos][int(num_str[pos])] += w
                    
                pair_weights[(int(num_str[0]), int(num_str[1]))] += w
                pair_weights[(int(num_str[2]), int(num_str[3]))] += w
                
    # Normalisasi Laplace
    pos_probs = [{} for _ in range(4)]
    for pos in range(4):
        total_pos_w = sum(pos_weights[pos].values()) or 1.0
        for d in range(10):
            pos_probs[pos][d] = (pos_weights[pos][d] + 0.1) / (total_pos_w + 1.0)
            
    total_pair_w = sum(pair_weights.values()) or 1.0
    total_double_w = sum(double_digit_freq.values()) or 1.0
        ...
        for d1 in range(10):
            for d2 in range(10):
                for d3 in range(10):
                    ...
                    # Pengganda Hasil Permutasi (Yield Multiplier)
                    # 24-way -> 1.0x, 12-way -> 2.0x, 6-way -> 4.0x, 4-way -> 6.0x
                    yield_multiplier = 24.0 / perms
                    ...
                    pair_f = (pair_weights.get((d0, d1), 0.0) + 0.01) / total_pair_w
                    pair_b = (pair_weights.get((d2, d3), 0.0) + 0.01) / total_pair_w
                    
                    # Bonus Digit Kembar jika digit tersebut memang berfrekuensi tinggi
                    double_bonus = 1.0
                    for d in set([d0, d1, d2, d3]):
                        if [d0, d1, d2, d3].count(d) >= 2:
                            double_bonus += (double_digit_freq.get(d, 0.0) / total_double_w) * 2.0
                            
                    expected_yield_score = base_prob * (pair_f ** 0.25) * (pair_b ** 0.25) * yield_multiplier * double_bonus
                    candidates.append((expected_yield_score, num_str))
...
def main():
    ...
    print("=" * 80)
    print(f" SIMULASI FORMULA 13: Permutation Yield Optimizer")
    ...
    for i, current_draw in enumerate(testing_draws):
        ...
        recs = generate_yield_maximizer_recommendations(historical_window, top_n=10)
        
        latest_recs_payload = {
            "formula_id": "13_permutation_yield_maximizer",
            "formula_name": "Permutation Yield Optimizer",
    ...
    print("=" * 80)
    print(" KEPUTUSAN PRESTASI PELABURAN 6 BULAN (FORMULA 13)")
    print("=" * 80)
