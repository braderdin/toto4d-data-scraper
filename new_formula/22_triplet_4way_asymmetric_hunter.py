...
MODULE       : 22_triplet_4way_asymmetric_hunter.py
FORMULA NAME : Triplet / 4-Way Asymmetric Yield Hunter
DESCRIPTION  : Mengoptimumkan jangkaan nilai asimetrik dengan memburu nombor famili
               4-way (3 digit serupa) dan 6-way (2 pasang kembar) yang memberikan
               pembahagi iBox terkecil (pulangan 4x hingga 6x ganda lebih tinggi).
STRATEGI BET : Dinamik RM18.00 (High Triplet Concentration) vs RM11.00 (Defensive).
...
TEMP_OUTPUT_FILE = os.path.join(TEMP_DIR, "recommendations_22_triplet_4way_asymmetric_hunter.json")
...
def generate_triplet_4way_recs(history_draws):
    """
    FORMULA 22: Triplet & 4-Way Yield Hunter + Dynamic Bet Sizing
    """
    ...
    for pos in range(4):
        ...
        for d in range(10):
            pos_probs[pos][d] = pos_alphas[pos][d] / tot_a
            
    tot_momentum = sum(digit_momentum.values()) or 1.0
    tot_pair_aff = sum(pair_affinity.values()) or 1.0
    
    # Kenalpasti 3 digit teratas yang paling padu momentumnya
    top_digits = sorted(digit_momentum.items(), key=lambda x: x[1], reverse=True)
    dominant_digit_power = (top_digits[0][1] / tot_momentum) if top_digits else 0.1
    
    # Ambang Keputusan: Jika kuasa digit teratas melepasi 13.5% daripada keseluruhan momentum
    is_high_concentration = dominant_digit_power >= 0.135
    conf_tier = "HIGH-TRIPLET (RM18)" if is_high_concentration else "DEFENSIVE (RM11)"
    
    triplet_4way_candidates = []
    twin_6way_candidates = []
    general_candidates = []
        ...
        for d1 in range(10):
            for d2 in range(10):
                for d3 in range(10):
                    ...
                    pf = pair_affinity.get((d0, d1), 0.5) / tot_pair_aff
                    pb = pair_affinity.get((d2, d3), 0.5) / tot_pair_aff
                    ...
                    general_candidates.append((base_log_p, num_str))
                    
                    # 4-Way Triplet (3 digit serupa): Pengganda Asimetrik Payout x6
                    if perms == 4:
                        triplet_score = base_log_p + math.log(24.0 / 4.0) + 0.6 * math.log(1.0 + dominant_digit_power)
                        triplet_4way_candidates.append((triplet_score, num_str))
                    # 6-Way (2 pasang kembar): Pengganda Asimetrik Payout x4
                    elif perms == 6:
                        twin_score = base_log_p + math.log(24.0 / 6.0) + 0.4 * math.log(1.0 + dominant_digit_power)
                        twin_6way_candidates.append((twin_score, num_str))
                        
    general_candidates.sort(key=lambda x: x[0], reverse=True)
    triplet_4way_candidates.sort(key=lambda x: x[0], reverse=True)
    twin_6way_candidates.sort(key=lambda x: x[0], reverse=True)
    
    recommendations = []
    seen = set()
    
    if is_high_concentration:
        # STRATEGI RM18: No 1-6 (Direct Big + iBox = RM12), No 7-12 (iBox 4-way/6-way = RM6)
        # 4 Calon Utama Asimetrik
        for _, num in triplet_4way_candidates:
            if num not in seen:
                seen.add(num)
                recommendations.append({"rank": len(recommendations) + 1, "number": num, "bet_direct_rm": 1, "bet_ibox_rm": 1})
            if len(recommendations) == 4:
                break
        # 2 Calon Umum Terbaik
        for _, num in general_candidates:
            ...
            if len(recommendations) == 6:
                break
        # 6 Calon Pelengkap 6-way / 4-way
        for _, num in twin_6way_candidates + triplet_4way_candidates:
            if num not in seen:
                seen.add(num)
        ...
        # STRATEGI RM11: No 1-3 (Direct Big + iBox = RM6), No 4-8 (iBox Sahaja = RM5)
        for _, num in triplet_4way_candidates[:2] + general_candidates[:3]:
            if num not in seen:
                seen.add(num)
                recommendations.append({"rank": len(recommendations) + 1, "number": num, "bet_direct_rm": 1, "bet_ibox_rm": 1})
    ...
    return recommendations, dominant_digit_power, conf_tier
...
def main():
    ...
    print("=" * 80)
    print(f" SIMULASI FORMULA 22: Triplet / 4-Way Asymmetric Yield Hunter")
    print(f" Jumlah Rekod: {total_records} | Latihan: {len(training_draws)} | Ujian: {len(testing_draws)}")
    print(f" Mod Taruhan Dinamik: Triplet-Dominance (RM18.00) vs Defensive (RM11.00)")
    print("=" * 80)
    ...
    for i, current_draw in enumerate(testing_draws):
        ...
        recs, power_val, conf_tier = generate_triplet_4way_recs(historical_window)
        ...
        latest_recs_payload = {
            "formula_id": "22_triplet_4way_asymmetric_hunter",
            "formula_name": "Triplet / 4-Way Asymmetric Yield Hunter",
            "target_date": target_date,
            "draw_no": draw_no,
            "confidence_tier": conf_tier,
            "dominant_power": round(power_val, 4),
    ...
    print("=" * 80)
    print(" KEPUTUSAN PRESTASI PELABURAN 6 BULAN (FORMULA 22)")
    print("=" * 80)
    print(f"  Jumlah Cabutan Diuji    : {len(testing_draws)}")
    print(f"  Sesi Triplet Dominan    : {high_tier_count}/{len(testing_draws)} cabutan")
