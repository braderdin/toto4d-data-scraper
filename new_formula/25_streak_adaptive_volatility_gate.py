...
MODULE       : 25_streak_adaptive_volatility_gate.py
FORMULA NAME : Streak-Adaptive Volatility Band
DESCRIPTION  : Sistem pengurusan risiko drawdown beradaptasi siri kalah-menang:
               - Hot Streak (Selepas Menang)     : RM 18.00 (12 Nombor)
               - Normal / Stabil (Kalah 1-3 Sesi): RM 11.00 (8 Nombor)
               - Dry Spell Gate (Kalah >= 4 Sesi): RM  6.00 (3 Nombor Ultra-Defensive)
...
TEMP_OUTPUT_FILE = os.path.join(TEMP_DIR, "recommendations_25_streak_adaptive_volatility_gate.json")
...
def generate_streak_adaptive_recs(history_draws, current_loss_streak):
    """
    FORMULA 25: Streak Adaptive Volatility Gate & Bayesian Core
    """
            ...
            for d2 in range(10):
                for d3 in range(10):
                    ...
                    if perms in (12, 6, 4):
                        ...
                        yield_log_p = log_p + math.log(24.0 / perms) + 0.45 * math.log(1.0 + db)
    ...
    # Penentuan Saiz Taruhan Mengikut Fasa Kitaran Kalah-Menang (Streak Gate)
    if current_loss_streak == 0:
        conf_tier = "HOT-STREAK (RM18)"
    elif current_loss_streak >= 4:
        conf_tier = f"CIRCUIT-BREAKER (RM6) [Streak Kalah={current_loss_streak}]"
    else:
        conf_tier = f"NORMAL-STABLE (RM11) [Streak Kalah={current_loss_streak}]"
    ...
    if "HOT" in conf_tier:
        ...
        for _, num in twin_candidates:
            ...
            if len(recommendations) == 12:
                break
    elif "CIRCUIT" in conf_tier:
        # RM6: No 1-3 (Direct Big + iBox = RM6 Sahaja - Mod Jimat Maksimum Semasa Fasa Kering)
        for _, num in general_candidates:
            if num not in seen:
                seen.add(num)
                recommendations.append({"rank": len(recommendations) + 1, "number": num, "bet_direct_rm": 1, "bet_ibox_rm": 1})
    ...
    return recommendations, conf_tier
...
def main():
    ...
    print("=" * 80)
    print(f" SIMULASI FORMULA 25: Streak-Adaptive Volatility Band")
    print(f" Jumlah Rekod: {total_records} | Latihan: {len(training_draws)} | Ujian: {len(testing_draws)}")
    print(f" Taruhan Beradaptasi Drawdown: Hot (RM18) | Normal (RM11) | Circuit-Breaker (RM6)")
    print("=" * 80)
    ...
    for i, current_draw in enumerate(testing_draws):
        ...
        recs, conf_tier = generate_streak_adaptive_recs(historical_window, current_loss_streak)
        
        cost_per_draw = sum(item['bet_direct_rm'] + item['bet_ibox_rm'] for item in recs)
        tier_counts[conf_tier.split()[0]] += 1
            
        latest_recs_payload = {
            "formula_id": "25_streak_adaptive_volatility_gate",
            "formula_name": "Streak-Adaptive Volatility Band",
        ...
        if winnings > 0:
            hits_count += 1
            current_loss_streak = 0  # Reset streak kalah
            status = f"[MENANG] +RM{winnings:8.2f} (Untung Bersih: RM{net_draw:+.2f})"
        else:
            current_loss_streak += 1 # Tambah streak kalah
    ...
    print("=" * 80)
    print(" KEPUTUSAN PRESTASI PELABURAN 6 BULAN (FORMULA 25)")
    print("=" * 80)
    print(f"  Jumlah Cabutan Diuji    : {len(testing_draws)}")
    for t_key, t_val in tier_counts.items():
        print(f"  - Mod {t_key:<20}: {t_val}/{len(testing_draws)} cabutan")
