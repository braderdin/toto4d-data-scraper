...
MODULE       : 03_markov_digit_chain.py
FORMULA NAME : Markov Transition Chain
DESCRIPTION  : Membina matriks kebarangkalian peralihan Markov tertib pertama
               antara cabutan berturutan untuk meramal digit seterusnya berdasarkan
               keadaan digit cabutan sebelumnya.
...
TEMP_OUTPUT_FILE = os.path.join(TEMP_DIR, "recommendations_03_markov_digit_chain.json")
...
def generate_markov_recommendations(history_draws, top_n=10):
    """
    FORMULA 03: Markov Transition Chain
    Mengira matriks peralihan Markov 10x10 untuk 4 posisi.
    """
    if len(history_draws) < 2:
        return []
        
    # Matriks transisi [posisi][digit_sebelum][digit_semasa]
    trans_counts = [{d_from: {d_to: 0.0 for d_to in range(10)} for d_from in range(10)} for _ in range(4)]
    ...
    # Bina matriks peralihan antara cabutan (t-1 -> t)
    for i in range(1, len(history_draws)):
        prev_draw = history_draws[i - 1]
        curr_draw = history_draws[i]
        
        # Ekstrak nombor utama
        prev_p1 = str(prev_draw.get('1st_prize', '')).strip()
        curr_items = []
        if curr_draw.get('1st_prize'): curr_items.append((curr_draw['1st_prize'], TIER_WEIGHTS['1st']))
        if curr_draw.get('2nd_prize'): curr_items.append((curr_draw['2nd_prize'], TIER_WEIGHTS['2nd']))
        if curr_draw.get('3rd_prize'): curr_items.append((curr_draw['3rd_prize'], TIER_WEIGHTS['3rd']))
        for sp in curr_draw.get('special_prizes', []): curr_items.append((sp, TIER_WEIGHTS['special']))
        for cs in curr_draw.get('consolation_prizes', []): curr_items.append((cs, TIER_WEIGHTS['consolation']))
        
        if len(prev_p1) == 4 and prev_p1.isdigit():
            for num_str, w in curr_items:
                num_str = str(num_str).strip()
                if len(num_str) == 4 and num_str.isdigit():
                    for pos in range(4):
                        d_prev = int(prev_p1[pos])
                        d_curr = int(num_str[pos])
                        trans_counts[pos][d_prev][d_curr] += w
                        
    # Normalisasi Matriks Transisi dengan Laplace Smoothing
    trans_probs = [{d_from: {} for d_from in range(10)} for _ in range(4)]
    for pos in range(4):
        for d_from in range(10):
            total_trans = sum(trans_counts[pos][d_from].values()) + 1.0
            for d_to in range(10):
                trans_probs[pos][d_from][d_to] = (trans_counts[pos][d_from][d_to] + 0.1) / total_trans

    # Ambil titik rujukan daripada cabutan terkini
    last_draw = history_draws[-1]
    ref_numbers = []
    if last_draw.get('1st_prize'): ref_numbers.append((str(last_draw['1st_prize']).strip(), 3.0))
    if last_draw.get('2nd_prize'): ref_numbers.append((str(last_draw['2nd_prize']).strip(), 2.0))
    if last_draw.get('3rd_prize'): ref_numbers.append((str(last_draw['3rd_prize']).strip(), 1.5))
    
    if not ref_numbers:
        ref_numbers = [("0000", 1.0)]
    ...
    for d0 in range(10):
        for d1 in range(10):
            for d2 in range(10):
                for d3 in range(10):
                    cand_digits = (d0, d1, d2, d3)
                    score = 0.0
                    for ref_str, weight in ref_numbers:
                        if len(ref_str) == 4 and ref_str.isdigit():
                            prob_prod = 1.0
                            for pos in range(4):
                                d_prev = int(ref_str[pos])
                                d_curr = cand_digits[pos]
                                prob_prod *= trans_probs[pos][d_prev][d_curr]
                            score += weight * prob_prod
...
def main():
    ...
    print("=" * 80)
    print(f" SIMULASI FORMULA 03: Markov Transition Chain")
    ...
    for i, current_draw in enumerate(testing_draws):
        ...
        recs = generate_markov_recommendations(historical_window, top_n=10)
        
        latest_recs_payload = {
            "formula_id": "03_markov_digit_chain",
            "formula_name": "Markov Transition Chain",
    ...
    print("=" * 80)
    print(" KEPUTUSAN PRESTASI PELABURAN 6 BULAN (FORMULA 03)")
    print("=" * 80)
