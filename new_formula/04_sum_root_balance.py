...
MODULE       : 04_sum_root_balance.py
FORMULA NAME : Sum & Digital Root Bell
DESCRIPTION  : Menapis kombinasi digit menggunakan lengkung taburan normal Gauss
               pada jumlah 4-digit (Digit Sum Bell Curve) dan keseimbangan punca
               digit (Digital Root 1-9).
...
TEMP_OUTPUT_FILE = os.path.join(TEMP_DIR, "recommendations_04_sum_root_balance.json")
...
def get_digital_root(n):
    """Mengira Punca Digital (Digital Root 1-9)."""
    if n == 0:
        return 0
    return 1 + ((n - 1) % 9)

def generate_sum_root_recommendations(history_draws, top_n=10):
    """
    FORMULA 04: Sum & Digital Root Bell
    Menjana kombinasi berasaskan lengkung Gauss Digit Sum & Punca Digit.
    """
    if not history_draws:
        return []
        
    historical_sums = []
    historical_roots = defaultdict(float)
    pos_digit_counts = [defaultdict(float) for _ in range(4)]
    ...
    for draw in history_draws:
        ...
        for num_str, w in items:
            ...
            if len(num_str) == 4 and num_str.isdigit():
                ...
                d_sum = sum(digits)
                d_root = get_digital_root(d_sum)
                
                historical_sums.append((d_sum, w))
                historical_roots[d_root] += w
                total_samples_weight += w
                
                for pos in range(4):
                    pos_digit_counts[pos][digits[pos]] += w
                    
    # Kira Min (Mu) dan Sisihan Piawai (Sigma) bagi Jumlah Digit
    if historical_sums:
        total_w = sum(w for _, w in historical_sums)
        mean_sum = sum(s * w for s, w in historical_sums) / total_w
        var_sum = sum(w * ((s - mean_sum) ** 2) for s, w in historical_sums) / total_w
        std_sum = math.sqrt(max(var_sum, 1.0))
    else:
        mean_sum = 18.0
        std_sum = 5.67
        
    # Normalisasi taburan Punca Digit
    dr_probs = {}
    for dr in range(1, 10):
        dr_probs[dr] = (historical_roots[dr] + 0.1) / (total_samples_weight + 0.9)
        
    # Normalisasi posisi
    pos_probs = [{} for _ in range(4)]
    for pos in range(4):
        total_pos_w = sum(pos_digit_counts[pos].values()) or 1.0
        for d in range(10):
            pos_probs[pos][d] = (pos_digit_counts[pos][d] + 0.1) / (total_pos_w + 1.0)
            
    # Penilaian skor bagi setiap kombinasi 4D
    candidates = []
    for d0 in range(10):
        for d1 in range(10):
            for d2 in range(10):
                for d3 in range(10):
                    ...
                    curr_root = get_digital_root(curr_sum)
                    
                    # Gaussian density: exp( - (sum - mu)^2 / (2 * sigma^2) )
                    gauss_factor = math.exp(- ((curr_sum - mean_sum) ** 2) / (2 * (std_sum ** 2)))
                    root_factor = dr_probs.get(curr_root, 0.01)
                    pos_factor = (
                        pos_probs[0][d0] * 
                        pos_probs[1][d1] * 
                        pos_probs[2][d2] * 
                    ...
                    score = gauss_factor * root_factor * (pos_factor ** 0.5)
...
def main():
    ...
    print("=" * 80)
    print(f" SIMULASI FORMULA 04: Sum & Digital Root Bell")
    ...
    for i, current_draw in enumerate(testing_draws):
        ...
        recs = generate_sum_root_recommendations(historical_window, top_n=10)
        
        latest_recs_payload = {
            "formula_id": "04_sum_root_balance",
            "formula_name": "Sum & Digital Root Bell",
    ...
    print("=" * 80)
    print(" KEPUTUSAN PRESTASI PELABURAN 6 BULAN (FORMULA 04)")
    print("=" * 80)
